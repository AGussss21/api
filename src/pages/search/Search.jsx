...
export default function ProductSearch() {
  ...
  const [query, setQuery] = useState(qParam); // initial dari qParam
  const sortRef = useRef(null);
  const priceRef = useRef(null);

  // sinkronkan query state setiap kali q param berubah (mis. pengunjung mengetik di hero SearchBar yang menavigasi ke /search?q=...)
  useEffect(() => {
    setQuery(qParam);
  }, [qParam]);

  // Filter & sort products (search across semua product: s- dan p-)
  const filteredProducts = useMemo(() => {
    ...
    if (q) {
      list = list.filter((p) => {
        const name = (p.nama || p.title || "").toLowerCase();
        const desc = (p.description || p.desc || "").toLowerCase();
        // cek juga kategori atau tag jika ada
    ...
    list = list.filter((p) => {
      ...
      return price >= Number(appliedMin) && price <= Number(appliedMax);
    });
    ...
    return list;
  }, [products, query, appliedMax, appliedMin, appliedSort]);
  ...
  // click outside handlers untuk menu
  useEffect(() => {
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative h-150 flex items-center justify-center">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 text-center text-white">
          ...
          >
            Cari produk Lola Cake — satuan & paketan, semua ada di sini.
          </motion.h1>
          ...
          >
            <div className="shadow-2xl rounded-full max-w-[760px] mx-auto">
              {/* SearchBar visual di hero.
                  Asumsi: komponen SearchBar sudah menavigasi ke /search?q=... saat submit.
                  Halaman ini hanya membaca q param dan menampilkan hasil. */}
              <SearchBar
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-center mt-2 max-w-xl mx-auto leading-relaxed">
          ...
          >
            {query ? `Menampilkan hasil untuk: "${query}"` : "Cari produk satuan atau paketan Lola Cake — gunakan kotak pencarian di atas."}
          </motion.p>
        </div>
        ...
        >
          {filteredProducts.length === 0 ? (
            ...
            >
              Produk belum ditemukan untuk kriteria pencarian ini.
            </motion.div>
          ) : (
            filteredProducts.map((p) => {
              const slug = `${slugify(p.nama || p.title)}`;
              // tentukan path detail berdasarkan prefix id
              const detailPath = String(p.id).startsWith("p-")
                ? `/product/paketan/${slug}`
                : `/product/satuan/${slug}`;
