...
import { products } from "../../data.js"; // pastikan file ada di src/data.js atau sesuaikan path. :contentReference[oaicite:1]{index=1}
...
export default function ProductSatuan() {
  ...
  // saat categories berubah, pastikan activeCategory valid
  useEffect(() => {
    if (categories && categories.length && !categories.find(c => c.id === activeCategory)) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (q) {
      list = list.filter((p) => (p.nama || p.title || "").toLowerCase().includes(q) || (p.desc || "").toLowerCase().includes(q));
    }
    list = list.filter((p) => Number(p.price) >= appliedMin && Number(p.price) <= appliedMax);

    // urutkan (buat salinan agar tidak mutate original)
    switch (appliedSort) {
      case "price-asc":
  ...
  }, [activeCategory, query, appliedMax, appliedMin, appliedSort]);
              ...
              {openPrice && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white shadow-lg rounded-lg p-4 z-20">
                  <div className="flex justify-between text-sm text-slate-700 mb-2">
                    <span className="text-slate-500">Rp{Number(appliedMin).toLocaleString()}</span>
                    <span className="text-slate-500">Rp500.000</span>
                  </div>
                  <input type="range" min={appliedMin} max={500000} step={1000} value={tempMax}
                  ...
                  <div className="flex justify-between gap-2 mt-4">
                    ...
                    <button onClick={() => { setAppliedMax(Number(tempMax)); setOpenPrice(false); }} className="px-3 py-1 rounded bg-[#f08b2d] text-white text-sm">
                      Terapkan
                    </button>
                  </div>
                </div>
